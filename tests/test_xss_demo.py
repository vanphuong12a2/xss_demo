...
import tempfile
import pytest
from xss_demo import xss_demo


@pytest.fixture
def client():
    db_fd, xss_demo.app.config['DATABASE'] = tempfile.mkstemp()
    client = xss_demo.app.test_client()
    with xss_demo.app.app_context():
        xss_demo.init_db()

    yield client

    os.close(db_fd)
    os.unlink(xss_demo.app.config['DATABASE'])
